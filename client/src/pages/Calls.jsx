      ...
      <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col font-sans">
        ...
        <div className="border-b border-zinc-800/80 pb-5 shrink-0">
          ...
          <p className="text-zinc-400 text-xs mt-1">
            Audit inbound call recordings, Whisper speech-to-text transcripts, and AI voice responses.
          </p>
        </div>
        ...
        <div className="flex-grow flex gap-6 min-h-0">
          ...
          <div className="flex-grow saas-panel p-5 flex flex-col min-h-0">
            ...
            {loading ? (
              ...
              <div className="flex-grow flex items-center justify-center border border-dashed border-zinc-800 rounded-xl p-8 text-center">
                <p className="text-zinc-500 text-xs">No phone calls registered yet.</p>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-zinc-800/60 text-xs">
                    {calls.map((c) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-3 pl-2 text-zinc-200">
                          ...
                          <td className="py-3 text-zinc-300">
                            {c.contact ? (
                              <span className="font-medium text-zinc-200">{c.contact.name}</span>
                            ) : (
                              <span className="italic text-zinc-500">Unlinked</span>
                            )}
                          </td>
                          ...
                          <td className="py-3 text-right pr-2">
                            {c.recordingUrl ? (
                              ...
                              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-zinc-900 border border-zinc-800 text-zinc-400">
                                Unavailable
                              </span>
                            )}
                          </td>
          ...
          <div className="w-[450px] saas-panel p-5 flex flex-col min-h-0 shrink-0 hidden lg:flex">
            {activeCallId ? (
              loadingDetails ? (
                <div className="flex-grow flex flex-col items-center justify-center space-y-3">
                  <div className="w-6 h-6 rounded-full border-2 border-indigo-500/20 border-t-indigo-500 animate-spin"></div>
                  <span className="text-xs text-zinc-400">Syncing Call logs...</span>
                </div>
              ) : (
                <div className="flex-grow flex flex-col min-h-0 space-y-5">
                  ...
                  <div className="border-b border-zinc-800/80 pb-3.5">
                    <h3 className="text-xs font-semibold font-heading text-zinc-300 tracking-wide uppercase mb-1.5">
                      Call Session Detail
                    </h3>
                    <div className="space-y-1 text-xs text-zinc-300">
                      <p><span className="text-zinc-400">Caller:</span> {callData?.contact?.name || 'Unknown'} ({callData?.contact?.phone || 'Not set'})</p>
                      <p><span className="text-zinc-400">Time:</span> {new Date(callData?.createdAt).toLocaleString()}</p>
                      <p><span className="text-zinc-400">Duration:</span> {formatDuration(callData?.callDuration)}</p>
                    </div>
                  </div>

                  {/* Audio Player */}
                  <div>
                    <h3 className="text-xs font-semibold font-heading text-zinc-300 tracking-wide uppercase mb-2.5">
                    ...
                    {callData?.recordingUrl ? (
                      ...
                      <div className="p-3.5 bg-zinc-950/60 border border-zinc-800/80 rounded-lg text-center text-xs text-zinc-400">
                  ...
                  {(callData?.summary || callData?.detectedOutcome) && (
                    <div className="bg-zinc-950/80 border border-zinc-800/80 rounded-xl p-3.5 space-y-2 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-zinc-400 uppercase tracking-wide text-[10px]">AI Call Summary</span>
                        ...
                        return (
                          <div key={index} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] rounded-xl px-3.5 py-2 text-xs leading-relaxed ${
                              ...
                              <span className="block text-[10px] uppercase font-semibold text-zinc-400 mb-0.5">
              ...
              <div className="flex-grow flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-zinc-400 text-xs max-w-xs">
                  Select a voice call log from the left table to review call transcripts and play recordings.
                </p>
              </div>
            )}
          </div>
